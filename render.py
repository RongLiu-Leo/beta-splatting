...
def rendering(args):
    ...
    scene = Scene(args, beta_model, args.iteration, shuffle=False)
    ...
    output_dir = os.path.join(
        args.model_path, "point_cloud", f"iteration_{args.iteration}", "renders"
    )
    gt_dir = os.path.join(
        args.model_path, "point_cloud", f"iteration_{args.iteration}", "gt"
    )
    ...
    def render_image(cameras, render_mode):
        for i, cam in enumerate(cameras):
            render_pkg = beta_model.render(cam, render_mode=render_mode)
            ...
            im = Image.fromarray((image_cpu * 255).astype("uint8"))
            im.save(os.path.join(gt_dir, f"RGB_{i}.png"))

    def render_video(cameras, render_mode, render_traj_path_type, fps=30):
        ...
        for i, cam in tqdm(enumerate(trajectory_cameras)):
            render_pkg = beta_model.render(cam, render_mode=render_mode)
            image = render_pkg["render"]
