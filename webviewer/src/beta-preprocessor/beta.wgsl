...
const sb_num_primitives = 2;
struct PointInput {
  @location(0) position: vec3<f32>,
  @location(1) log_scale: vec3<f32>,
  @location(2) rot: vec4<f32>,
  @location(3) c0: vec3<f32>,
  @location(4) opacity_logit: f32,
  @location(5) beta: f32,
  sb_params: array<vec3<f32>, sb_num_primitives * 2>, // Each primitive has [r,g,b] and [theta,phi,beta]
};

struct AuxData {
  num_intersections: atomic<u32>,
  num_visible_betas: atomic<u32>,
...
struct BetaData {
  id: i32,
  radii: i32,  // Also signals whether this Beta is in frustum.
  depth: f32,
  tiles_touched: u32,
...
@group(0) @binding(2) var<storage, read_write> result: array<BetaData>;
...
fn main(
  ...
  // of screen-space tiles that this Beta overlaps with. Quit if
  ...
  let radius_px: i32 = i32(ceil(sqrt(max(lambda_1, lambda_2))));
  ...
  let color = point.c0 + compute_color_from_sb(world_pos.xyz, point.sb_params, point.beta);
  ...
  atomicAdd(&auxData.num_visible_betas, 1u);
...
fn compute_cov2d(position: vec3<f32>, log_scale: vec3<f32>, rot: vec4<f32>) -> vec3<f32> {
  ...
  // Apply low-pass filter: every Beta should be at least
...
fn compute_color_from_sb(position: vec3<f32>, sb_params: array<vec3<f32>, sb_num_primitives * 2>, beta_sharpness: f32) -> vec3<f32> {
  let dir = normalize(position - uniforms.camera_position);
  
  // Base color (equivalent to c0 in the CUDA implementation)
  var result = vec3<f32>(0.0, 0.0, 0.0);
  
  // Iterate through spherical beta primitives
  for (var i = 0; i < sb_num_primitives; i++) {
    // Extract parameters: r, g, b are in sb_params[i*2]
    // theta, phi, beta are in sb_params[i*2+1]
    let r = sb_params[i*2].x;
    let g = sb_params[i*2].y;
    let b = sb_params[i*2].z;
    let theta = sb_params[i*2+1].x;
    let phi = sb_params[i*2+1].y;
    let primitive_beta = sb_params[i*2+1].z;
    
    // Compute direction vector of primitive
    let dir_beta_mean = vec3<f32>(
      sin(theta) * cos(phi),
      sin(theta) * sin(phi),
      cos(theta)
    );
    ...
    let dot_val = max(0.0, dot(dir, dir_beta_mean));
    
    // Skip if dot product is zero
    if (dot_val > 0.0) {
      let exponent = 4.0 * exp(primitive_beta);
      let betaTerm = pow(dot_val, exponent);
      
      // Add color contribution
      result += vec3<f32>(r, g, b) * betaTerm;
    }
  }
